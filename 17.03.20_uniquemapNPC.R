...
# npc_TE <- merge(npc_TE, TE_classification[,c(1,2)], by.x='row.names', by.y='TE_id')
# npc_TE <- npc_TE[,-1]
# npc_TE <- aggregate(npc_TE[,-ncol(npc_TE)], by=list(npc_TE$TE_subfamily), FUN=sum)
# rownames(npc_TE) <- npc_TE$Group.1
# npc_TE <- npc_TE[,-1]
# npc_TE <- npc_TE[which(rowSums(npc_TE) > 0),]
